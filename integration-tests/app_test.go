...
func TestUdpWithPion(t *testing.T) {
	ctx := context.Background()
	var logConsumer LogConsumer

	// docker run -p 127.0.0.1::3478/udp stun:latest
	exposedPort, err := nat.NewPort(udpProto, stunPort)
	if err != nil {
		t.Error(err)
	}
	exposedPortRequest := fmt.Sprintf("127.0.0.1::%s/%s", exposedPort.Port(), exposedPort.Proto())

	expectedStartLog := fmt.Sprintf("Starting Stun server, listening port at %s/%s", stunPort, udpProto)
	...
	serverAddr, err := net.ResolveUDPAddr(udpProto, serverUri)
	if err != nil {
	...
	fmt.Printf("Container should see request arriving at network: %s from ip: %s\n", networks[0], expectedMappedIp)
...
func TestTcpWithPion(t *testing.T) {
	ctx := context.Background()
	var logConsumer LogConsumer

	// docker run -p 127.0.0.1::3478/tcp stun:latest
	...
	serverAddr, err := net.ResolveTCPAddr(tcpProto, serverUri)
	if err != nil {
		log.Fatalln("failed to resolve addr: ", serverUri, "with error: ", err)
	}
